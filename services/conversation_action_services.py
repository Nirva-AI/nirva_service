...
###################################################################################################################################################################
conversation_action_router = APIRouter()


###################################################################################################################################################################
###################################################################################################################################################################
###################################################################################################################################################################
@conversation_action_router.post(
    path="/action/conversation/v1/", response_model=ConversationActionResponse
)
async def handle_conversation(
    ...
    logger.info(f"/action/conversation/v1/: {request_data.model_dump_json()}")
    ...
    if not user_session_manager.has_user_session(request_data.user_name):
        logger.error(
            f"action/conversation/v1: {request_data.user_name} has no session, please login first."
        )
    ...
    if current_user_session.chat_system is None:
        logger.error(
            f"action/conversation/v1: {request_data.user_name} has no chat_system, please login first."
        )
        return ConversationActionResponse(
            error=1002,
            message="没有游戏，请先登录",
