...
from nirva_service.db.crypt_context import verify_password
from nirva_service.db.jwt import (
    UserToken,
    create_access_token,
...
from .oauth_user import get_authenticated_user, oauth2_scheme
...
async def login(
    ...
    try:
        # 检查用户是否存在
        if not nirva_service.db.pgsql_user.has_user(form_data.username):
            raise HTTPException(
        ...
        # 获取用户信息
        user_db = nirva_service.db.pgsql_user.get_user(form_data.username)
        if not user_db:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="用户名或密码错误",
        ...
        # 将令牌存储到 Redis 中
        nirva_service.db.redis_user.assign_user_access_token(
...
async def refresh(refresh_token: str = Form(...)) -> UserToken:
    try:
        ...
        # 检查用户是否存在
        if not nirva_service.db.pgsql_user.has_user(username):
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="无效的刷新令牌",
                headers={"WWW-Authenticate": "Bearer"},
...
async def logout(
    ...
    try:
        ...
        if "jti" in payload:
            ...
            # 将令牌添加到黑名单
            nirva_service.db.redis_user.add_access_token_to_blacklist(
                str(jti), remaining_time
            )

        # 选择性操作：删除用户在Redis中的令牌信息（使所有设备登出）
        # 取消注释下面的代码以启用此功能
        nirva_service.db.redis_user.remove_user_access_token(current_user)
