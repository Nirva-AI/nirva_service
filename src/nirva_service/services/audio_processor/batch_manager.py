...
class BatchManager:
    ...
    def __init__(self, max_gap_seconds: int = 30, timeout_seconds: int = 120):
        """
        Initialize batch manager.
        
        Args:
            max_gap_seconds: Maximum gap between segments to consider same batch
            timeout_seconds: Time after which to process a batch
        """
...
def get_batch_manager(
    ...
    if _batch_manager is None:
        max_gap = max_gap_seconds or int(os.getenv('BATCH_MAX_GAP_SECONDS', '30'))
        timeout = timeout_seconds or int(os.getenv('BATCH_TIMEOUT_SECONDS', '120'))
