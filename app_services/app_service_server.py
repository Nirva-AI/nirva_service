...
_app_service_server_instance: Optional[AppserviceServer] = None
...
def _initialize_langgraph_service() -> LanggraphService:
    ...
    return LanggraphService(
        chat_service_localhost_urls=[
            f"http://localhost:{chat_server_config.port}{chat_server_config.api}"
        ],
        analyzer_service_localhost_urls=[
            f"http://localhost:{analyzer_server_config.port}{analyzer_server_config.api}"
        ],
    )


###############################################################################################################################################
def get_app_service_server_instance() -> AppserviceServer:

    global _app_service_server_instance
    if _app_service_server_instance is None:
        _app_service_server_instance = AppserviceServer(
            langgraph_service=_initialize_langgraph_service(),
        )

    assert (
        _app_service_server_instance is not None
    ), "UserSessionServer instance is not initialized."
    return _app_service_server_instance


###############################################################################################################################################
AppserviceServerInstance = Annotated[
    AppserviceServer, Depends(get_app_service_server_instance)
]
